...
int main()
{
  ...
  printf(ANSI_COLOR_YELLOW   "EXPERIMENT 4: MCTS WITH CUT COMPUTATION" ANSI_COLOR_RESET "\n" );
  ...
  if( ENC == 0 )
  {
    ...
    kitty::create_from_binary_string( tt, ISTR );
    F=tt;
  }
  else if( ENC == 1 )
  {
    ...
    kitty::dynamic_truth_table tt( nVars );
    kitty::create_from_hex_string( tt, ISTR );
    F=tt;
  }
  else
  {
    ...
    printf(ANSI_COLOR_YELLOW " METHOD: " ANSI_COLOR_RESET "\n" );
    printf(ANSI_COLOR_YELLOW "    [0] RANDOM COVERING " ANSI_COLOR_RESET "\n" );
    printf(ANSI_COLOR_YELLOW "    [1] MCTS COVERING " ANSI_COLOR_RESET "\n" );
    int MET;
    std::cin >> MET;
      ...
      double duration;
      start = std::clock();
      report_t<xag_network> rep = game_on<xag_network>(&F, MET);
