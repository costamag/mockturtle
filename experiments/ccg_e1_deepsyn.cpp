...
int main()
{
  ...
  for( uint32_t iFn{0}; iFn<2; ++iFn )
  {
    printf("%s\n", fNames[iFn].c_str() );
    printf("%5s %5s || %5s | %5s | %5s \n", "n", "thr", "N", "N*" "T[s]" );
    for( uint32_t nVars{2}; nVars < 20; ++nVars )
    {
      int iThr = ( floor( nVars/2 ) + 1 );
        kitty::dynamic_truth_table F;
        F = propose_game( nVars, iThr, iFn );

        report_t<aig_network> rep = game_on<aig_network>(&F, 0, 33 ); // set to random sampling 1 for 33 iterations @.@
        aig_network aig = rep.ntk;
        ...
        dot_file  = "EXPS/EXP1/" + fNames[iFn] +"/dot/"  + sName + ".dot";
        blif_file = "EXPS/EXP1/" + fNames[iFn] +"/blif/" + sName + ".blif";
        aig_file  = "EXPS/EXP1/" + fNames[iFn] +"/aig/"  + sName + ".aig";
        write_dot( aig, dot_file );
        write_dot( aig, blif_file );
        write_aiger( aig, aig_file );
        dot_file  = "EXPS/EXP1/" + fNames[iFn] +"/dot/"  + sName + "rs.dot";
        blif_file = "EXPS/EXP1/" + fNames[iFn] +"/blif/" + sName + "rs.blif";
        aig_file  = "EXPS/EXP1/" + fNames[iFn] +"/aig/"  + sName + "rs.aig";
        write_dot( aig_resub, dot_file );
        write_dot( aig_resub, blif_file );
        write_aiger( aig_resub, aig_file );
      printf("%5d %5d || %5d | %5d | %5.2f \n", nVars, iThr, aig.num_gates(), aig_resub.num_gates(), rep.time );
    }
  }
  return 0;
}
