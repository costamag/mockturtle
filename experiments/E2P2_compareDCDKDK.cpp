  ...
  for( auto x : combinations )
  {
    ...
    std::reverse(x.begin(), x.end());
      ...
      if( result == lorina::return_code::success )
      {
        ...
        klut_network oklut2;
        simulation_view oklut_sim2{ oklut2 };
        std::vector<kitty::partial_truth_table> tgt2 = {ytt};
        model M2( oklut_sim2, Xtt, tgt2 );
        hdc::detail::arecovery_method arecovery_m2 = hdc::detail::arecovery_method::ixtsdecS;
        hdc::detail::arecovery_params arecovery_ps2;
        arecovery_ps2.verbose = false;
        arecovery_ps2.output=0;
      ...
      selection_ps.max_new_supports = 60;
      selection_ps.max_selection_attempts = 300;
      ...
      creation_ps.max_nodes_total  = 60;

      creation_ps.output=0;
      M2.add( selection_m, selection_ps, creation_m, creation_ps );

        auto oSig2 = M2.accuracy_recovery(arecovery_m2,arecovery_ps2);
        oklut_sim2.create_po(oSig2);
        oklut2 = oklut_sim2;
        partial_simulator sim_sim2( pats );
        unordered_node_map<kitty::partial_truth_table, klut_network> node_to_value_sim2( oklut2 );
        simulate_nodes( oklut2, node_to_value_sim2, sim_sim2 );
        double adk2 = ((double)(kitty::count_zeros(node_to_value_sim2[oklut2.po_at(0)]^Y))*100/16);

        ADC[ADC.size()-1]+=adc;
        ADK[ADK.size()-1]+=adk;
        ADK2[ADK2.size()-1]+=adk2;
        Nacc++;
      }
      else
...
void test_n_var_function( )
{
  ...
  do
  {
    ...
    else
    {
      ...
      ADK2[ADK2.size()-1] = ADK2[ADK2.size()-1]/Nacc;
...
int main()
{
  ...
  {
    std::cout << ADC[i] << " " << ADK[i] << " " << ADK2[i] << std::endl;
  }
